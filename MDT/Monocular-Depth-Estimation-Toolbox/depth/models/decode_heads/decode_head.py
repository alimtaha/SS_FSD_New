...
class DepthBaseDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        if self.classify:
            assert bins_strategy in [
                "UD", "SID"], "Support bins_strategy: UD, SID"
            assert norm_strategy in [
                "linear", "softmax", "sigmoid"], "Support norm_strategy: linear, softmax, sigmoid"
            ...
            self.conv_depth = nn.Conv2d(
                channels, 1, kernel_size=3, padding=1, stride=1)
    ...
    def forward_train(self, img, inputs, img_metas, depth_gt, train_cfg):
        ...
        log_imgs = self.log_images(
            img[0], depth_pred[0], depth_gt[0], img_metas[0])
    ...
    def depth_pred(self, feat):
        ...
        if self.classify:
            ...
            output = torch.einsum(
                'ikmn,k->imn', [logit, bins]).unsqueeze(dim=1)
