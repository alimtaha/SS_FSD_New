...
class DenseDepthHead(DepthBaseDecodeHead):
    ...
    def __init__(self,
        ...
        if self.fpn:
            ...
            for idx, in_channel in enumerate(
                    self.in_channels[:self.num_fpn_levels]):
    ...
    def forward(self, inputs, img_metas):
        ...
        if self.fpn:
            ...
            for index, feat in enumerate(inputs[::-1]):
                ...
                # Change align corners to True.
                if index != 0:
                    y = cur_fpn + \
                        F.interpolate(temp_feat_list[-1], size=cur_fpn.shape[-2:], mode="bilinear", align_corners=True)
                else:
                    y = cur_fpn
