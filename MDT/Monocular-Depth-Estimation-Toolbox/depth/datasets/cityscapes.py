...
class CSDataset(Dataset):
    ...
    def format_results(
        ...
        results[0] = (
            results[0] *
            self.depth_scale)  # Do not convert to np.uint16 for ensembling. # .astype(np.uint16)
        return results
    ...
    def eval_kb_crop(self, depth_gt):
        ...
        depth_cropped = depth_gt[top_margin: top_margin +
    ...
    def pre_eval(self, preds, indices):
        ...
        for i, (pred, index) in enumerate(zip(preds, indices)):
            ...
            depth_map_gt = mmcv.imresize(
                depth_map_gt, (2048, 1024), return_scale=False)
            ...
            print(type(depth_map_gt))
            print('mask shape', valid_mask.shape)
            print('depth map shape', depth_map_gt.shape)
