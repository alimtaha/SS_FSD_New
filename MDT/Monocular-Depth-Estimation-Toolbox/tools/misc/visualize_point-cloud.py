...
def main():
    ...
    for idx, input in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            name = osp.splitext(img_file)[0].split(
                '/')[-2] + '_' + osp.splitext(img_file)[0].split('/')[-1]
        ...
        pix_coords = torch.cat(
            [id_coords[0].view(-1).unsqueeze(dim=0), id_coords[1].view(-1).unsqueeze(dim=0)], 0)
        ...
        img_tensor = torch.tensor(
            img[44:480 - 44, 40:640 - 40, :], dtype=torch.uint8)
        img_tensor = img_tensor[:, :, [2, 1, 0]]

        img_tensor_flatten = img_tensor.permute(2, 0, 1).flatten(start_dim=1)

        generate_pointcloud_ply(
            cam_points,
            img_tensor_flatten.numpy(),
            os.path.join(
                args.output_dir,
                name + '.ply'))
        progress_bar.update()
