...
class SequenceSegmentationLoader(data.Dataset):
    def __init__(
        ...
        for i in range(self.num_scales):
            ...
            self.resize[i] = transforms.Resize(
                (self.crop_h // s, self.crop_w // s), interpolation=Image.ANTIALIAS)
        s = 2 ** color_full_scale
        self.resize_full = transforms.Resize(
        ...
        if len(self.files) == 0:
            raise RuntimeError(
                f"Found no segmentation files in {self.images_base}")

        self._filter_available_files()

        if self.split == "train" and self.restrict_dict is not None:
            self.files = restrict_to_subset(
                self.files,
                **self.restrict_dict,
                seed=self.seed,
                load_labeled=self.load_labeled,
                load_unlabeled=self.load_unlabeled)
        ...
        if not self.files or len(self.files) == 0:
            raise Exception(
                f"No files for split={self.split} found in {self.images_base}")
    ...
    def get_teacher_depth(self, index, do_flip):
        ...
        if self.generated_depth_dir:
            ...
            depth = pil_loader(
                depth_path, -1, -1, is_segmentation=True, lru_cache=True)
            if do_flip:
    ...
    def __getitem__(self, index):
        ...
        if self.load_labels:
            if self.load_onehot and is_labeled:
                ...
                onehot_lbl = torch.nn.functional.one_hot(
                    dense_lbl, self.n_classes + 2)
                onehot_lbl = onehot_lbl[..., :self.n_classes]
                onehot_lbl = onehot_lbl.permute(2, 0, 1)
            elif self.load_onehot and not is_labeled:
                onehot_lbl = torch.zeros(
                    (self.n_classes, *inputs["lbl"].shape)).long()
            else:
                onehot_lbl = None
            if self.load_onehot:
