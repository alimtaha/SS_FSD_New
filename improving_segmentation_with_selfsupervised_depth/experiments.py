...
def lr_schedule(cfg, lr_sch, max_iter, step=30e3):
    if lr_sch == "step":
        ...
        cfg["training"]["lr_schedule"] = {
            "name": "multi_step", "milestones": [
                int(30e3), int(40e3), int(50e3)], "gamma": 0.5}
    elif lr_sch == "step30":
        cfg["training"]["lr_schedule"] = {
...
def setup_dataset(cfg, dataset, crop, lr_sch):
    train_iters = {
        "cityscapes": int(40e3),
        "mapillary": int(40e3),
        "camvid": int(20e3)}[dataset]
    step = {
        "cityscapes": int(30e3),
        "mapillary": int(30e3),
        "camvid": int(15e3)}[dataset]
    final_val_interval = {
        "cityscapes": 500,
        "mapillary": 1000,
        "camvid": 500}[dataset]
...
def generate_experiment_cfgs(base_cfg, id):
            ...
            for seed in [
                ...
                for n_subset in subsets(dataset):
                    ...
                    for name, seg_init, teacher_init, ema, mix_mask, only_unlabeled, mix_use_gt, preselect, mix_video in [
                        ...
                        ('scratch_classmix', 'none', 'none', True,
                        ...
                        ('transfer', mono_pretrain, mono_pretrain,
                        ...
                        unlab_str = "" if not ema else f"_Unlab{unlab_cfg['consistency_weight']}{unlab_cfg['mix_mask']}" + (
                            "jit" if unlab_cfg["color_jitter"] else "") + ("blur" if unlab_cfg["blur"] else "")
                        ...
                        cfg = setup_optimizer(
                            cfg, opt, lr, blr, None, None, gclip)
    ...
    # Semi-Supervised Segmentation with Multi-Task Learning
        ...
        mono_lambda = 1
        seg_lambda = 1
        dec, dec_params, crop, batch_size = (
            6, "lr5_fd2_crop512x512bs4", (512, 512), 2)
        for seed in [
            # 7,
            # 25,
            ...
            for n_subset in subsets(dataset):
                ...
                for name, ema, mix_mask, only_unlabeled, mix_use_gt, preselect in [
                    ...
                    unlab_cfg = {
                        ...
                        "depthcomp_margin": dc_m,
                        "depthcomp_foreground_threshold": dc_ft,
                        "debug_image": True} if ema else None
                    unlab_str = "" if not ema else f"_Unlab{unlab_cfg['consistency_weight']}{unlab_cfg['mix_mask']}" + \
