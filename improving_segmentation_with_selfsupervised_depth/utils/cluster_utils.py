    ...
    def _extract_resolved_base_vars(cfg, cfgs):
        ...
        for k, v in cfg.items():
            ...
            if isinstance(v, dict):
                for part_k, part_v in CustomVariantGenerator._extract_resolved_base_vars(
                        cfg[k], [c[k] for c in cfgs]).items():
    ...
    def _generate_trials(self, num_samples, unresolved_spec, output_path=""):
        ...
        for _ in range(num_samples):
            ...
            for unresolved_cfg in unresolved_spec["config"]:
                ...
                resolved_base_vars = CustomVariantGenerator._extract_resolved_base_vars(
                    unresolved_cfg, unresolved_spec["config"])
                print("Resolved base cfg vars", resolved_base_vars)
                for resolved_vars, spec in generate_variants(
                        unresolved_spec_variant):
                    ...
                    if resolved_vars:
                        experiment_tag += "_{}".format(format_vars(
                            {k: v for k, v in resolved_vars.items() if "tag" in k}))
                    self._counter += 1
