...
class MonodepthLoss:
    ...
    def generate_depth_test_pred(self, outputs):
        assert outputs[("disp", 0)].shape[-2:] == (self.height,
                                                   self.width), outputs[("disp", 0)].shape[-2:]
        for scale in self.scales:
            ...
            _, depth = disp_to_depth(
                disp, self.test_min_depth, self.test_max_depth)
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        """
        assert outputs[("disp", 0)].shape[-2:] == (self.height,
                                                   self.width), f'{outputs[("disp", 0)].shape[-2:]} should be {(self.height, self.width)} '
        for scale in self.scales:
            disp = outputs[("disp", scale)]
            disp = F.interpolate(
                disp, [
