...
class Inference():
    def __init__(self, cfg, logdir, run_id):
        ...
        if "monodepth_options" in cfg:
            ...
            cfg['model']['depth_args']['max_scale_size'] = (
                cfg["monodepth_options"]["crop_h"], cfg["monodepth_options"]["crop_w"])
    ...
    def load_resume(self, strict=True):
        if os.path.isfile(self.cfg["training"]["resume"]):
            ...
            print(
                f"WARNING: load_resume - {self.cfg['training']['resume']} not found")
    ...
    def run(self):
        ...
        with torch.no_grad():
            for inputs_val in tqdm(self.val_data_loader,
                ...
                for filename, img, seg, depth in zip(
                        inputs_val["filename"], images_val, pred, outputs[("disp", 0)]):
                    fn = f"{self.logdir}/{filename}"
                    os.makedirs(os.path.dirname(fn), exist_ok=True)
                    save_image(img, fn)
                    if depth is not None:
                        save_image(depth, fn.replace(".jpg", "_depth.png"))
                    ps_lab_col = torch.tensor(
                        self.val_loader.decode_segmap_tocolor(seg)).permute(
                        2, 0, 1)
