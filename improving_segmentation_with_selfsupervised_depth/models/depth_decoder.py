    ...
    def __init__(
        ...
        for i in range(self.n_upconv, -1, -1):
            ...
            if i == self.n_upconv and intermediate_aspp:
                self.convs[("upconv", i, 0)] = ASPP(
                    num_ch_in, aspp_rates, aspp_pooling, num_ch_out)
            else:
                self.convs[("upconv", i, 0)] = ConvBlock(
                    num_ch_in, num_ch_out, bn=batch_norm, dropout=dropout)
    ...
    def forward(self, input_features, x=None, exec_layer=None):
        ...
        for i in range(self.n_upconv, -1, -1):
            ...
            if x.shape[-1] < input_features[i - 1].shape[-1] or i == 0:
                ...
                x = [x]
            if self.use_skips and i > 0:
                projected_features = self.convs[("skip_proj", i)](
                    input_features[i - 1])
