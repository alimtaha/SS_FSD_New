    ...
    if backbone_name in ["resnet18", "resnet50", "resnet101"]:
        ...
        if backbone_pretraining == "none":
            ...
            encoder_path = os.path.join(
                MachineConfig.DOWNLOAD_MODEL_DIR,
                backbone_pretraining,
                "encoder.pth")
            loaded_dict_enc = torch.load(
                encoder_path, map_location=torch.device(device))
            filtered_dict_enc = {
                k: v for k,
                v in loaded_dict_enc.items() if k in backbone.state_dict()}
        ...
        if use_intermediate_layer_getter:
            ...
            backbone = IntermediateLayerGetter(
                backbone.encoder, return_layers=return_layers)
    else:
        raise NotImplementedError
...
def get_posenet(
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    models = {}
    models["pose_encoder"] = get_resnet_backbone(
    ...
    if "mono" in pose_pretraining:
        for mn in ["pose_encoder", "pose"]:
            ...
            filtered_dict = {
                k: v for k,
                v in loaded_dict.items() if k in models[mn].state_dict()}
