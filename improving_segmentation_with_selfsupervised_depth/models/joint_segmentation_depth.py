...
class JointSegmentationMonodepth(nn.Module):
    ...
    def predict_poses(self, inputs, features):
        ...
        if self.num_pose_frames == 2:
            ...
            if self.provide_uncropped_for_pose:
                pose_feats = {f_i: inputs["color_full_aug", f_i, 0]
            ...
            for f_i in self.frame_ids[1:]:
                if f_i != "s":
                    # To maintain ordering we always pass frames in temporal
                    # order
                    if f_i < 0:
            ...
            # Here we input all frames to the pose net (and predict all poses)
            # together
            if self.provide_uncropped_for_pose:
    ...
    def forward(self, x):
        ...
        if "imnet_encoder" in self.models:
            ...
            with torch.no_grad():
                outputs["imnet_features"] = self.models["imnet_encoder"](
                    inputs["color_aug", 0, 0])[-1].detach()
...
def get_segmentation_network(
    ...
    segmentation_net = model_map[segmentation_name](
        num_ch_enc,
        num_ch_dec,
        num_classes,
        **segmentation_args,
...
def joint_segmentation_depth(
        ...
        depth_args,
        disable_monodepth,
        enable_imnet_encoder,
        disable_pose,
        imnet_encoder_dilation=True,
        **kwargs):
    ...
    if segmentation_name in ["mtl_pad"]:
        models["mtl_decoder"] = get_segmentation_network(
            segmentation_name,
            num_ch_enc,
            (height,
             width),
        ...
        if not disable_monodepth:
            models["depth"] = get_depth_decoder(
                depth_pretraining, num_ch_enc, range(num_scales), **depth_args)
        if segmentation_name is not None:
            models["segmentation"] = get_segmentation_network(
                segmentation_name,
                num_ch_enc,
                (height,
                 width),
    ...
    model = JointSegmentationMonodepth(
        models,
        frame_ids,
        use_pose_net,
        num_pose_frames,
