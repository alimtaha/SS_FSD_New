...
class JointSegDepthDecoder(nn.Module):
    ...
    def forward(self, encoder_features):
        ...
        segmentation_size = _get_layer(
            encoder_features, seg_features, self.last_layer).shape[2:]
        last_layer_size = tuple(
            np.array(segmentation_size) //
            self.output_stride)
        stacked_features = []
        for layer in self.layers:
            if f"seg{layer}" in self.project:
                proj_own = self.project[f"seg{layer}"](
                    _get_layer(encoder_features, seg_features, layer))
                if JointSegDepthDecoder.first_iter:
                    print(f'proj_seg{layer}', proj_own.shape)
                proj_own = F.interpolate(proj_own, size=last_layer_size,
                                         mode='bilinear', align_corners=False)
                if JointSegDepthDecoder.first_iter:
                    print(f'proj_seg{layer} upsampled', proj_own.shape)
        ...
        if JointSegDepthDecoder.first_iter:
            print(f'score before interpolate', score.shape)
...
class PAD(nn.Module):
    ...
    def __init__(
        ...
        num_scales = 4
        self.depth_dec = get_depth_decoder(
            weights, num_ch_enc, range(num_scales), **depth_args)
        self.seg_dec = get_depth_decoder(
            weights, num_ch_enc, range(num_scales), **depth_args)
    ...
    def forward(self, encoder_features):
        ...
        first_exec_layers = list(
            range(self.dec_n_upconv, dec_distill_i - 1, -1))
        second_exec_layers = list(range(dec_distill_i - 1, -1, -1))

        # Initial predictions
        if PAD.first_iter:
            print(f"PAD run first half of decoder ({first_exec_layers}).")
        depth_features = self.depth_dec(
            encoder_features, exec_layer=first_exec_layers)
        seg_features = self.seg_dec(
            encoder_features,
            exec_layer=first_exec_layers)
        if self.side_output:
            intermediate_seg = self.seg_intermediate_head(
                seg_features[intermediate_layer_name])

        # Self-attention
        features_sa_depth = self.sa_depth(
            depth_features[intermediate_layer_name])
        features_sa_seg = self.sa_seg(seg_features[intermediate_layer_name])
        if PAD.first_iter:
            print(
                f"PAD self attention for {intermediate_layer_name} with shape {features_sa_depth.shape}.")

        # Feature stack
        merged_for_seg = seg_features[intermediate_layer_name] + \
            features_sa_depth
        merged_for_depth = depth_features[intermediate_layer_name] + \
            features_sa_seg
        ...
        depth_features.update(
            self.depth_dec(
                encoder_features,
                x=merged_for_depth,
                exec_layer=second_exec_layers))
        seg_features = self.seg_dec(
            encoder_features,
            x=merged_for_seg,
            exec_layer=second_exec_layers)
        final_seg = self.seg_final_head(
            _get_layer(
                features_sa_depth,
                seg_features,
                self.final_layer))
