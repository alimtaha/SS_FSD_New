...
def label_selection_main(cfg):
    ...
    last_from_scratch = cfg["label_selection"].get(
        "last_from_scratch", train_from_scratch)
    label_steps = cfg["label_selection"]["label_steps"]
    train_iters_per_step = [int(v)
                            for v in cfg["label_selection"]["train_iters"]]
    ...
    if train_from_scratch:
        ...
        train_iters_until_step = [
            sum(train_iters_per_step[0:i + 1]) for i in range(len(train_iters_per_step))]
    if choice in ["ifp", "random"]:
        assert last_from_scratch or train_from_scratch
        if sum([v for k, v in cfg["label_selection"].items()
                if "lambda" in k]) == 0:
    ...
    if resume_file != "":
        ...
        print(
            f"LABEL_SELECTION: Resume at step {resume_step} from {resume_file} with samples {labeled_samples}")
    for i, (n_new_subset, train_iters) in enumerate(
            zip(label_steps, train_iters_until_step)):
        is_last_step = (i == len(label_steps) - 1)
        if i < resume_step:
            continue
        if i == 0:
            labeled_samples = choose_initial_samples(
                cfg, n_new_subset, mode=initial_samples)
        else:
            print(f"LABEL_SELECTION: Evaluate model {model_file}")
            labeled_samples = choose_new_samples(
                cfg, model_file, labeled_samples, n_new_subset, choice)

        if train_iters == 0:
            continue
        print(
            f"LABEL_SELECTION: Train on {len(labeled_samples)} samples: {labeled_samples}")
        ...
        if is_last_step and cfg["label_selection"].get(
                "last_segmentation_only", False):
            current_cfg['training']['pseudo_depth_lambda'] = 0
            current_cfg['training']['monodepth_lambda'] = 0
        if is_last_step and cfg["label_selection"].get(
                "last_depth_only", False):
            current_cfg['training']['segmentation_lambda'] = 0
        model_file = train_on_subset(
            current_cfg,
            labeled_samples,
            train_iters,
            model_file_to_continue,
            tensorboard_in_subdir=train_from_scratch or last_from_scratch)
        # If tensorboard_in_subdir is wrong: # find . -type f -wholename
        # "*nlabels*/*.metrics" -execdir mv -t ../ {} +
...
def build_trainer(cfg, experiment_name, tensorboard_in_subdir=True):
    ...
    cfg['training']['disable_depth_estimator'] = not is_first_trainer or cfg['training'].get(
        'disable_depth_estimator', False)

    if tensorboard_in_subdir:
        writer = SummaryWriter(log_dir=log_dir,
                               filename_suffix=f'{experiment_name}.metrics')
        img_writer = SummaryWriter(
            log_dir=log_dir,
            filename_suffix=f'{experiment_name}.tensorboardimgs')
    else:
...
def choose_initial_samples(cfg, n, mode):
    if mode == "random":
        ...
        labeled_samples = choose_new_samples(
            ifp_cfg, None, seed_sample, n, "ifp")
    ...
    if isinstance(scores[0]['label_criterion'], list):
        ...
        for i in range(n_criteria):
            sorted_scores = sorted(
                scores,
                key=lambda k: k['label_criterion'][i],
                reverse=True)
        ...
        scores = sorted(
            scores,
            key=lambda k: k['label_criterion'],
            reverse=True)
...
def choose_samples_from_ifp(
    ...
    if preselection_multiplier is not None:
        ...
        presorted_scores = sorted(
            scores,
            key=lambda k: k['label_criterion'][0],
            reverse=True)
        preselected_samples = [s["idx"].item() for s in presorted_scores[:int(
            preselection_multiplier * n_to_add)]]
        print("LABEL_SELECTION: Preselected samples:", preselected_samples)
    idxs, ifp_distances = iterative_farthest_point(
        initial_samples, feature_distances, n_to_add, preselected_samples)
    chosen_samples, chosen_scores = [], []
...
def choose_new_samples(
    ...
    assert n_to_add > 0
    n_random_choice_all = cfg["label_selection"].get(
        "n_random_choice_all", get_n_total(cfg))
    preselection_multiplier = cfg["label_selection"]["preselection_multiplier"]
    with np_local_seed(cfg["seed"]):
        all_samples = np.random.permutation(np.arange(get_n_total(cfg)))[
            :n_random_choice_all].tolist()
    ...
    if choice in ["ifp"]:
        ...
        if preselection_multiplier is not None:
            assert sum(
                [v for k, v in cfg["label_selection"].items() if "lambda" in k]) != 0
        chosen_samples, chosen_scores = choose_samples_from_ifp(
            current_samples, scores, feat_distances, n_to_add, preselection_multiplier)
    elif choice == "score":
        # If all entropy lambdas are zero, we assume random mode
        if not isinstance(cfg['label_selection']['entropy_lambda'], list) and sum(
                [v for k, v in cfg["label_selection"].items() if "lambda" in k]) == 0:
            chosen_samples = unlabeled_samples[:n_to_add]
        else:
            scores, _ = acquire_scores(
                cfg, unlabeled_samples, all_samples, model_file)
            chosen_samples, chosen_scores = choose_samples_from_scores(
                scores, n_to_add)
    else:
    ...
    assert len(new_subset) == len(
        set(new_subset)), f"Subset contains duplicates: {sorted(new_subset)}"

    if debug:
        logs, _ = acquire_scores(
            cfg, chosen_samples, all_samples, model_file, verbose=True)
        if choice in ["ifp"]:
            _, logs = choose_samples_from_ifp(
                current_samples, logs, feat_distances, n_to_add, preselection_multiplier)
        else:
            _, logs = choose_samples_from_scores(logs, n_to_add)
        rows, cols = 3, 3
        log_dir = os.path.join(
            os.path.join(
                cfg["training"]["log_path"],
                f"new_labels_{n_new_subset}"))
        os.makedirs(log_dir)
        for i, log in enumerate(logs):
            ...
            axs[0][1].imshow(
                log["disparity"][0].cpu().numpy(),
                cmap="plasma_r")
            axs[0][2].imshow(
                log["teacher_depth"][0].cpu().numpy(),
                cmap="plasma_r")
            axs[1][2].imshow(
                log["depth_error_map"].cpu().numpy(),
                cmap="plasma")
            axs[1][0].imshow(
                Cityscapes.decode_segmap_tocolor(
                    log["segmentation_pred"]))
            axs[1][1].imshow(
                Cityscapes.decode_segmap_tocolor(
                    log["segmentation_gt"]))
            axs[2][0].imshow(
                log["segmentation_entropy"].cpu().numpy(),
                cmap="viridis")
            ...
            plt.savefig(
                os.path.join(
                    log_dir,
                    f"new_label_{i}_{log['used_label_criterion']}.jpg"))
            plt.close()
...
def acquire_scores(
    ...
    with torch.no_grad():
        ...
        for inputs in tqdm(trainer.train_data_loader):
            ...
            if calc_depth_distances:
                ...
                if ifp_args["m"] in ["aspp", "u3", "u4", "bn"]:
                    ...
                    depth_features = pool_fn(
                        depth_features, (ifp_args["h"], 2 * ifp_args["h"]))
                elif ifp_args["m"] == "logdepth":
                    depth_features = inputs["pseudo_depth"][0]
                    depth_features = torch.log(
                        torch.clamp(1 / depth_features, 0.1, 80))
                    depth_features = pool_fn(
                        depth_features, (ifp_args["h"], 2 * ifp_args["h"]))
                ...
                dist_i_to_img_idx[len(all_depth_features)
                                  ] = inputs["idx"].item()
                img_idx_to_dist_i[inputs["idx"].item()] = len(
                    all_depth_features)
            ...
            depth_error_maps = []
            depth_errors = []
            depth_error_types = cfg["label_selection"].get(
                "depth_error_types", "abs")
            ...
            for depth_error_type in depth_error_types:
                if depth_error_type == "abs":
                    ...
                    depth_pseudo = torch.log(
                        torch.clamp(1 / disp_pseudo, 0.1, 80))
                    ...
                    depth_error_map = torch.abs(
                        disp_pred - disp_pseudo) / (disp_pseudo + 1e-1)
                elif depth_error_type == "sq_rel":
                    depth_error_map = (
                        (disp_pred - disp_pseudo) ** 2) / (disp_pseudo + 1e-1)
            ...
            assert not (isinstance(depth_lambda, list)
                        and len(depth_error_types) > 1)
            if isinstance(depth_lambda, list):
                label_criterion = []
                for dl, el in zip(depth_lambda, entropy_lambda):
                    label_criterion.append(
                        (dl * depth_error + el * entropy_mean).detach())
                ...
                for depth_error in depth_errors:
                    label_criterion.append(
                        (depth_lambda * depth_error + entropy_lambda * entropy_mean).detach())
            else:
                label_criterion = (
                    depth_lambda * depth_error + entropy_lambda * entropy_mean).detach()
            ...
            if verbose:
                ...
                trainer.monodepth_loss_calculator_val.generate_depth_test_pred(
                    mono_outputs)

                # Crop away bottom of image with own car
                if depth_loss_mask is None:
                    depth_loss_mask = torch.ones(
                        outputs["disp", 0].shape, device=trainer.device)
                    depth_loss_mask[:, :, int(
        ...
        if calc_depth_distances:
            depth_feature_distances = _calc_feature_distance(
                all_depth_features,
                dist_bias,
                dist_bias_weight,
                p=ifp_args["p"],
                normalize_features=ifp_args.get(
                    "norm",
                    False),
                patch_wise=ifp_args.get(
                    "pw",
                    False))
        feature_distances = depth_ifp_w * depth_feature_distances

    return scores, {'distances': feature_distances,
                    'dist_i_to_img_idx': dist_i_to_img_idx, 'img_idx_to_dist_i': img_idx_to_dist_i}


def _calc_feature_distance(
        features,
        bias,
        bias_weight,
        p,
    ...
    if patch_wise:
        ...
        for j in range(N_CHUNKS):
            ...
            assert chunk_distances.shape == (
                current_chunk_size * H * W, N * H * W)
            print(chunk_distances.shape)
            chunk_distances = chunk_distances.reshape(
                current_chunk_size, H * W, N, H * W)
            chunk_distances = chunk_distances.permute(0, 2, 1, 3)
            assert chunk_distances.shape == (
                current_chunk_size, N, H * W, H * W)
            print(chunk_distances.shape)
    ...
    if bias_weight > 0:
        ...
        feature_distances += torch.tensor(bias,
...
def iterative_farthest_point(
    ...
    if preselected_samples is not None:
        preselected_samples = [img_idx_to_dist_i[s]
                               for s in preselected_samples]
        ignored_samples = [
            i for i in range(
                dist.shape[0]) if i not in preselected_samples]
