...
# causes omp infos in contrast to tensorboardX
...
def get_train_params(model, cfg):
    ...
    if "segmentation_lr" in cfg["training"]["optimizer"]:
        train_params.append(
            {
                'params': remaining_params["segmentation"],
                'lr': cfg["training"]["optimizer"]["segmentation_lr"]})
...
def current_val_interval(cfg, step):
    v_intervals = [(int(k), int(v))
                   for k, v in cfg["training"]["val_interval"].items()]
...
class Trainer():
    def __init__(self, cfg, writer, img_writer, logger, run_id):
        ...
        if "generated_depth_dir" in cfg["data"]:
            ...
            assert not (depth_teacher and cfg['model'].get(
                'detph_estimator_weights') is not None)
            if depth_teacher is not None:
                cfg["data"]["generated_depth_dir"] += dataset_name + \
                    "/" + depth_teacher + "/"
            else:
                cfg["data"]["generated_depth_dir"] += dataset_name + \
                    "/" + cfg['model']['depth_estimator_weights'] + "/"
        ...
        if self.cfg["training"].get(
            "minimize_entropy_unlabeled",
                False) or self.enable_unlabled_segmentation:
            ...
            if self.only_unlabeled:
                unlabeled_segmentation_cfg.update(
                    {"load_unlabeled": True, "load_labeled": False})
            elif self.only_labeled:
                unlabeled_segmentation_cfg.update(
                    {"load_unlabeled": False, "load_labeled": True})
            else:
                unlabeled_segmentation_cfg.update(
                    {"load_unlabeled": True, "load_labeled": True})
            if self.mix_video:
                ...
                unlabeled_segmentation_cfg.update(
                    {"only_sequences_with_segmentation": not self.mix_video, "restrict_to_subset": None})
            self.unlabeled_loader = build_loader(
                unlabeled_segmentation_cfg,
                "train",
        ...
        # monodepth dataloader settings uses drop_last=True and shuffle=True
        # even for val
        ...
        # print(self.model)
        assert not (
            self.enable_unlabled_segmentation and self.cfg["training"]["save_monodepth_ema"])
        if self.enable_unlabled_segmentation and not self.only_labeled:
            ...
            # TODO: Try to remove unnecessary components and fit into gpu for
            # better performance
        ...
        optimizer_params = {
            k: v for k,
            v in cfg["training"]["optimizer"].items() if k not in [
                "name",
                "backbone_lr",
                "pose_lr",
                "depth_lr",
    ...
    def create_ema_model(self, model):
        ...
        for i in range(0, n):
            mcp[i].data[:] = mp[i].to(
                mcp[i].device, non_blocking=True).data[:].clone()
        return ema_model

    def update_ema_variables(self, ema_model, model, alpha_teacher, iteration):
        if self.cfg["training"]["save_monodepth_ema"]:
            model_params, ema_params = self.extract_monodepth_ema_params(
                model, ema_model)
        elif self.cfg['model']['segmentation_name'] == 'mtl_pad':
            model_params, ema_params = self.extract_pad_ema_params(
                model, ema_model)
        else:
            ...
            for img, label in zip(images.numpy(), labels.numpy()):
                if num_saved < self.cfg["training"]["n_tensorboard_trainimgs"]:
                    ...
                    self.img_writer.add_image(
                        ...
                        global_step=0)
                    colored_image = self.val_loader.decode_segmap_tocolor(
                        label)
                    self.img_writer.add_image(
    ...
    def train_step(self, inputs, step):
        ...
        if self.enable_unlabled_segmentation:
            ...
            for k in unlabeled_inputs.keys():
                if "color_aug" in k or "K" in k or "inv_K" in k or "color" in k or k in [
                        "onehot_lbl", "pseudo_depth"]:
                    # print(f"Move {k} to gpu.")
                    unlabeled_inputs[k] = unlabeled_inputs[k].to(
        ...
        if self.cfg["training"]["monodepth_lambda"] > 0:
            ...
            self.monodepth_loss_calculator_train.generate_images_pred(
                inputs, outputs)
            mono_losses = self.monodepth_loss_calculator_train.compute_losses(
                inputs, outputs)
            ...
            if feat_dist_lambda > 0:
                feat_dist = torch.dist(
                    outputs["encoder_features"],
                    outputs["imnet_features"],
                    p=2)
        ...
        if self.cfg["training"].get("pseudo_depth_lambda", 0) > 0:
            ...
            with torch.no_grad():
                depth_loss_mask = torch.ones(
                    outputs["disp", 0].shape, device=self.device)
                depth_loss_mask[:, :, int(
                    outputs["disp", 0].shape[2] * 0.9):, :] = 0
            pseudo_depth_loss = berhu(
                outputs["disp", 0], inputs["pseudo_depth"], depth_loss_mask)
        ...
        if self.cfg["training"]["segmentation_lambda"] > 0:
            with autocast(enabled=self.cfg["training"]["amp"]):
                segmentation_loss = self.loss_fn(
                    input=outputs["semantics"], target=inputs["lbl"])
                if "intermediate_semantics" in outputs:
                    segmentation_loss += self.loss_fn(
                        input=outputs["intermediate_semantics"], target=inputs["lbl"])
                    segmentation_loss /= 2
            ...
            if self.enable_unlabled_segmentation:
                unlabeled_loss, unlabeled_mono_loss = self.train_step_segmentation_unlabeled(
                    unlabeled_inputs, step)
        ...
        if self.cfg["training"].get("clip_grad_norm") is not None:
            ...
            if self.cfg["training"].get("disable_depth_grad_clip", False):
                torch.nn.utils.clip_grad_norm_(
                    get_params(
                        self.model, [
                            "encoder", "segmentation"]), self.cfg["training"]["clip_grad_norm"])
            else:
                torch.nn.utils.clip_grad_norm_(
                    self.model.parameters(),
                    self.cfg["training"]["clip_grad_norm"])
        # optimizer's gradients are already unscaled, so scaler.step does not unscale them,
        # although it still skips optimizer.step() if the gradients contain
        # infs or NaNs.
        ...
        if self.ema_model is not None:
            self.ema_model = self.update_ema_variables(
                ema_model=self.ema_model, model=self.model, alpha_teacher=0.99, iteration=step)
    ...
    def setup_segmentation_unlabeled(self):
        ...
        self.depthmix_online_depth = unlabeled_cfg.get(
            "depthmix_online_depth", False)
    ...
    def generate_mix_mask(self, mode, argmax_u_w, unlabeled_imgs, depths):
        if mode == "class":
            ...
            for image_i, other_image_i in [(0, 1), (1, 0)]:
                ...
                foreground_mask = torch.ge(
                    own_disp, other_disp - self.depthcomp_margin).long()
                # Avoid hiding the real background of the other image with own
                # a bit closer background
                if isinstance(
                        self.depthcomp_foreground_threshold,
                        tuple) or isinstance(
                        self.depthcomp_foreground_threshold,
                        list):
                    ft_l, ft_u = self.depthcomp_foreground_threshold
                    assert ft_u > ft_l
                    ft = torch.rand(1, device=own_disp.device) * \
                        (ft_u - ft_l) + ft_l
                else:
            ...
            for image_i in range(self.cfg["training"]["batch_size"]):
                ...
                min_depth = 0.1
                max_depth = 0.4
                depth_threshold = torch.rand(
                    1, device=depths.device) * (max_depth - min_depth) + min_depth
                if image_i == 0:
                    MixMask = transformmasks.generate_depth_mask(
                        generated_depth, depth_threshold).cuda()
                else:
                    MixMask = torch.cat(
                        (MixMask, transformmasks.generate_depth_mask(
                            generated_depth, depth_threshold).cuda()))
            ...
            for image_i in range(self.cfg["training"]["batch_size"]):
                generated_depth = depths[image_i]
                hist, bin_edges = np.histogram(torch.log(
                    1 + generated_depth).flatten(), bins=100, density=True)
                ...
                for v, e in zip(hist, bin_edges):
                    if v > 0.4:
                        ...
                        break
                depth_threshold = torch.rand(
                    1) * (max_depth - min_depth) + min_depth
                if image_i == 0:
                    MixMask = transformmasks.generate_depth_mask(
                        generated_depth, depth_threshold).cuda()
                else:
            ...
            MixMask = torch.ones(
                (unlabeled_imgs.shape[0], *unlabeled_imgs.shape[2:]), device=self.device)
        else:
    ...
    def calc_pseudo_label_loss(self, teacher_softmax, student_logits):
        ...
        unlabeled_weight = torch.sum(max_probs.ge(
            0.968).long() == 1).item() / np.prod(pseudo_label.shape)
        pixelWiseWeight = unlabeled_weight * \
            torch.ones(max_probs.shape, device=self.device)
    ...
    def train_step_segmentation_unlabeled(self, unlabeled_inputs, step):
        def strongTransform(parameters, data=None, target=None):
            ...
            data, target = transformsgpu.mix(
                mask=parameters["Mix"], data=data, target=target)
            data, target = transformsgpu.color_jitter(
                jitter=parameters["ColorJitter"], data=data, target=target)
            data, target = transformsgpu.gaussian_blur(
                blur=parameters["GaussianBlur"], data=data, target=None)
            return data, target
        ...
        if self.depthmix_online_depth:
            ...
            if self.cfg["training"]["monodepth_lambda"] > 0:
                self.monodepth_loss_calculator_train.generate_images_pred(
                    unlabeled_inputs, outputs_1)
                mono_losses = self.monodepth_loss_calculator_train.compute_losses(
                    unlabeled_inputs, outputs_1)
                mono_lambda = self.cfg["training"]["monodepth_lambda"]
                mono_loss = mono_lambda * mono_losses["loss"]
                self.scaler.scale(mono_loss).backward(
                    retain_graph=self.unlabeled_backward_first_pseudo_label)
            ...
            if self.unlabeled_backward_first_pseudo_label:
                ...
                L_1, _ = self.calc_pseudo_label_loss(
                    teacher_softmax=softmax_u_w, student_logits=logits_1)
        ...
        # Third Step: Run Mix
        MixMask = self.generate_mix_mask(
            self.mix_mask, argmax_u_w, unlabeled_imgs, depths)
        ...
        softmax_u_w_mixed, _ = strongTransform(
            strong_parameters, data=softmax_u_w)
        L_2, pseudo_label = self.calc_pseudo_label_loss(
            teacher_softmax=softmax_u_w_mixed, student_logits=logits_u_s)
        ...
        for j, (f, img, ps_lab, mask, d) in enumerate(
            ...
            axs[1][0].imshow(
                self.val_loader.decode_segmap_tocolor(
                    ps_lab.cpu().numpy()))
            for ax in axs.flat:
                ax.axis("off")
            plt.savefig(fn)
            ...
            for inputs in self.train_data_loader:
                ...
                if (step + 1) % self.cfg["training"]["print_interval"] == 0:
                    ...
                    self.writer.add_scalar(
                        ...
                        step + 1)
                    self.writer.add_scalar(
                        "training/time_per_image",
                        time_meter.avg /
                        self.cfg["training"]["batch_size"],
                        step +
                        1)
                    self.writer.add_scalar(
                        "training/amp_scale", self.scaler.get_scale(), step + 1)
                    self.writer.add_scalar(
                        "training/memory",
                ...
                if (step + 1) % current_val_interval(self.cfg, step +
                                                     1) == 0 or (step + 1) == self.cfg["training"]["train_iters"]:
                    self.validate(step)
    ...
    def validate(self, step):
        ...
        with torch.no_grad():
            for inputs_val in tqdm(self.val_data_loader,
                ...
                if self.cfg["training"]["segmentation_lambda"] > 0:
                    ...
                    val_segmentation_loss = self.loss_fn(
                        input=semantics, target=labels_val)
                ...
                if not self.cfg["model"]["disable_monodepth"]:
                    if not self.cfg["model"]["disable_pose"]:
                        self.monodepth_loss_calculator_val.generate_images_pred(
                            inputs_val, outputs)
                        mono_losses = self.monodepth_loss_calculator_val.compute_losses(
                            inputs_val, outputs)
                        val_mono_loss = mono_losses["loss"]
                    else:
                        outputs.update(
                            self.model.predict_test_disp(inputs_val))
                        self.monodepth_loss_calculator_val.generate_depth_test_pred(
                            outputs)
                ...
                if self.cfg["data"].get("depth_teacher", None) is not None:
                    ...
                    with torch.no_grad():
                        ...
                        depth_loss_mask[:, :, int(
                            outputs["disp", 0].shape[2] * 0.9):, :] = 0
                    val_pseudo_depth_loss = berhu(outputs["disp",
                                                          0],
                                                  inputs_val["pseudo_depth"],
                                                  depth_loss_mask,
                                                  apply_log=self.cfg["training"].get("pseudo_depth_loss_log",
                                                                                     False))
                else:
                ...
                for img, label, output, depth in zip(
                        images_val, gt, pred, outputs[("disp", 0)]):
                    if len(
                            imgs_to_save) < self.cfg["training"]["n_tensorboard_imgs"]:
                        imgs_to_save.append([
                            img, label, output,
        ...
        if self.cfg["training"]["segmentation_lambda"] > 0:
            ...
            for k, v in class_iou.items():
                self.writer.add_scalar(
                    "val_metrics/cls_{}".format(k), v, step + 1)
        ...
        for j, imgs in enumerate(imgs_to_save):
            ...
            if imgs[3] is not None:
                colored_image = _colorize(
                    imgs[3], "plasma", max_percentile=100)
                self.img_writer.add_image(
                    "{}/{}_3depth".format(
                        self.run_id.replace(
                            '/',
...
def train_main(cfg):
    ...
    img_writer = SummaryWriter(
        log_dir=logdir,
        filename_suffix='.tensorboardimgs')
