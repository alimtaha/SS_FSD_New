...
if __name__ == '__main__':
    ...
    for epoch in range(train_opt.epoch_count, train_opt.nepoch + 1):
        ...
        if train_opt.use_sne:
            another = tensor2im(
                (tempDict['another_image'] + 1) / 2)    # color normal images
        else:
        ...
        # show training images in tensorboard
        writer.add_image('Epoch' + str(epoch), image_numpy, dataformats='HWC')

        print('End of epoch %d / %d \t Time Taken: %d sec' %
              (epoch, train_opt.nepoch, time.time() - epoch_start_time))
        ...
        with torch.no_grad():
            for i, data in enumerate(valid_dataset):
                ...
                conf_mat += confusion_matrix(gt,
                                             pred,
                                             valid_dataset.dataset.num_labels)
                losses = model.get_current_losses()
                valid_loss_iter.append(model.loss_segmentation)
                print(
                    'valid epoch {0:}, iters: {1:}/{2:} '.format(
                        epoch,
                        epoch_iter,
                        len(valid_dataset) *
                        valid_opt.batch_size),
                    end='\r')
        ...
        # Save the best model according to the F-score, and record
        # corresponding epoch number in tensorboard
