...
class RoadSegModel(BaseModel):
    ...
    def initialize(self, opt, dataset):
        ...
        # specify the training losses you want to print out. The program will
        # call base_model.get_current_losses
        self.loss_names = ['segmentation']
        # specify the images you want to save/display. The program will call
        # base_model.get_current_visuals
        self.visual_names = ['rgb_image', 'another_image', 'label', 'output']
        # specify the models you want to save to the disk. The program will
        # call base_model.save_networks and base_model.load_networks
        self.model_names = ['RoadSeg']

        # load/define networks
        self.netRoadSeg = networks.define_RoadSeg(
            dataset.num_labels,
            use_sne=opt.use_sne,
        ...
        self.criterionSegmentation = networks.SegmantationLoss(
            class_weights=None).to(self.device)

        if self.isTrain:
            # initialize optimizers
            self.optimizers = []
            self.optimizer_RoadSeg = torch.optim.SGD(
                self.netRoadSeg.parameters(),
                lr=opt.lr,
    ...
    def get_loss(self):
        self.loss_segmentation = self.criterionSegmentation(
