...
def get_scheduler(optimizer, opt):
    if opt.lr_policy == 'lambda':
        def lambda_rule(
            epoch): return opt.lr_gamma ** ((epoch + 1) // opt.lr_decay_epochs)
...
def init_weights(net, init_type='normal', gain=0.02):
    ...
    def init_func(m):
        ...
        if hasattr(
            ...
            if hasattr(
                    m,
                    'bias') and m.bias is not None and init_type != 'pretrained':
...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[]):
    if len(gpu_ids) > 0:  # if GPU ids set to -1 this whole if block should be ignored
    ...
    for root_child in net.children():
        ...
        for children in root_child.children():
            print(root_child.children())
            if children in net.need_initialization:  # changed root_children to net
...
class RoadSeg(nn.Module):
    ...
    def __init__(self, num_labels, use_sne):
        ...
        if self.num_resnet_layers == 18:
            ...
            raise NotImplementedError(
                'num_resnet_layers should be 18, 34, 50, 101 or 152')

        ### encoder for another image ###
        if use_sne:
            self.encoder_another_conv1 = resnet_raw_model1.conv1
        else:
            # if another image is depth, initialize the weights of the first
            # layer
            self.encoder_another_conv1 = nn.Conv2d(
                1, 64, kernel_size=7, stride=2, padding=3, bias=False)
            self.encoder_another_conv1.weight.data = torch.unsqueeze(
        ...
        self.conv1_1 = conv_block_nested(
            filters[0] * 2, filters[0], filters[0])
        self.conv2_1 = conv_block_nested(
            filters[1] * 2, filters[1], filters[1])
        self.conv3_1 = conv_block_nested(
            filters[2] * 2, filters[2], filters[2])
        self.conv4_1 = conv_block_nested(
            filters[3] * 2, filters[3], filters[3])

        self.conv1_2 = conv_block_nested(
            filters[0] * 3, filters[0], filters[0])
        self.conv2_2 = conv_block_nested(
            filters[1] * 3, filters[1], filters[1])
        self.conv3_2 = conv_block_nested(
            filters[2] * 3, filters[2], filters[2])

        self.conv1_3 = conv_block_nested(
            filters[0] * 4, filters[0], filters[0])
        self.conv2_3 = conv_block_nested(
            filters[1] * 4, filters[1], filters[1])

        self.conv1_4 = conv_block_nested(
            filters[0] * 5, filters[0], filters[0])
    ...
    def forward(self, rgb, another):
        ...
        x1_3 = self.conv1_3(
            torch.cat([x1_0, x1_1, x1_2, self.up2_2(x2_2)], dim=1))
        x2_3 = self.conv2_3(
            torch.cat([x2_0, x2_1, x2_2, self.up3_2(x3_2)], dim=1))

        x1_4 = self.conv1_4(
            torch.cat([x1_0, x1_1, x1_2, x1_3, self.up2_3(x2_3)], dim=1))
        out = self.final(x1_4)
        return out
