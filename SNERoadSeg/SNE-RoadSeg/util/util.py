        ...
        if label == 'output':
            if prob_map:
                ...
                cv2.imwrite(os.path.join(
                    save_dir, image_name[:-10] + 'road_' + image_name[-10:]), im)
            else:
                im = tensor2labelim(im_data, impalette)
                im = cv2.resize(im, oriSize)
                cv2.imwrite(
                    os.path.join(
                        save_dir, image_name), cv2.cvtColor(
                        im, cv2.COLOR_RGB2BGR))
...
def tensor2confidencemap(label_tensor, imtype=np.uint8):
    ...
    label_image = np.true_divide(
        softmax_numpy[1],
        softmax_numpy[0] +
        softmax_numpy[1])
    label_image = np.floor(255 *
                           (label_image -
                            label_image.min()) /
                           (label_image.max() -
                               label_image.min()))
...
def confusion_matrix(x, y, n, ignore_label=None, mask=None):
    ...
    return np.bincount(n * x[k].astype(int) + y[k],
...
def getScores(conf_matrix):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        classrecall = np.diag(conf_matrix) / \
            conf_matrix.sum(1).astype(np.float)
        IU = np.diag(conf_matrix) / (conf_matrix.sum(1) + \
                     conf_matrix.sum(0) - np.diag(conf_matrix)).astype(np.float)
        pre = classpre[1]
