...
if __name__ == '__main__':
    ...
    # was 1, set to 0 as there is no multiprocessing support on Windows (and
    # potentially Mac)
    opt.num_threads = 0
    opt.batch_size = 1
    ...
    with torch.no_grad():
        for i, data in enumerate(dataset):
            ...
            save_images(
                ...
                model.get_image_names(),
                model.get_image_oriSize(),
                opt.prob_map)
            ...
            print('Epoch {0:}, iters: {1:}/{2:}, loss: {3:.3f} '.format(opt.epoch,
                                                                        epoch_iter, len(dataset) * opt.batch_size, test_loss_iter[-1]), end='\r')

        avg_test_loss = torch.mean(torch.stack(test_loss_iter))
        print(
            'Epoch {0:} test loss: {1:.3f} '.format(
                opt.epoch,
                avg_test_loss))
        globalacc, pre, recall, F_score, iou = getScores(conf_mat)
        print(
            'Epoch {0:} glob acc : {1:.3f}, pre : {2:.3f}, recall : {3:.3f}, F_score : {4:.3f}, IoU : {5:.3f}'.format(
                opt.epoch,
                globalacc,
                pre,
                recall,
