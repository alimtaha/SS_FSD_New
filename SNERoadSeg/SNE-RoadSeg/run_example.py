...
if __name__ == '__main__':
    ...
    # if you want to use your own data, please modify rgb_image, depth_image,
    # camParam and use_size correspondingly.
    ...
    # resize image to enable sizes divide 32 - THIS MIGHT POSE A PROBLEM IF
    # IMAGES NEED TO BE 1248 X 384, DO THEY UPSAMPLE IN THEIR DATALOADER?
    use_size = (1248, 384)
    rgb_image = cv2.resize(rgb_image, use_size)  # LOOKS LIKE UPSAMPLE
    rgb_image = rgb_image.astype(np.float32) / 255

    # compute normal using SNE
    sne_model = SNE()
    camParam = torch.tensor([[7.215377e+02,
                              0.000000e+00,
                              6.095593e+02],
                             [0.000000e+00,
                              7.215377e+02,
    ...
    cv2.imwrite(os.path.join('examples', 'normal.png'), cv2.cvtColor(
        255 * (1 + normal_image) / 2, cv2.COLOR_RGB2BGR))
