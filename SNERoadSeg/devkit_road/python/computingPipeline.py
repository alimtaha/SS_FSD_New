...
#     print "Using ground truth data from: %s" % trainData_path_gt
    ...
    for cat in dataStructure.cats:
        ...
        for testData_file_im2 in testData_fileList_im2:
            ...
            offset = abs(cur_image[:,
                                   :,
                                   0].astype('f4') - target_colorValue[0]) / 128. + abs(cur_image[:,
                                                                                                  :,
                                                                                                  1].astype('f4') - target_colorValue[1]) / 128. + abs(cur_image[:,
                                                                                                                                                                 :,
                                                                                                                                                                 2].astype('f4') - target_colorValue[2]) / 128
            cur_conf = np.clip(1. - offset / 3, 0., 1.)

            if visFlag:
                visImage = overlayImageWithConfidence(
                    cur_image, cur_conf, threshold=threshold)
