...
def main(train_dir, test_dir, outputDir):
    ...
    trainData_path_gt = os.path.join(
        train_dir, dataStructure.trainData_subdir_gt)
    ...
    for cat in dataStructure.cats:
        ...
        trainData_fileList_gt = sorted(
            glob(
                os.path.join(
                    trainData_path_gt,
                    cat +
                    '*' +
                    dataStructure.gt_end)))
        assert len(
            trainData_fileList_gt) > 0, 'Error: Cannot find ground truth data in %s' % trainData_path_gt
        ...
        for trainData_fileName_gt in trainData_fileList_gt:
            ...
            assert locationPotential.shape[0] >= trainData_file_gt.shape[
                0], 'Error: Y dimension of locationPotential is too small: %d' % trainData_file_gt.shape[0]
            assert locationPotential.shape[1] >= trainData_file_gt.shape[
                1], 'Error: X dimension of locationPotential is too small: %d' % trainData_file_gt.shape[1]

            locationPotential[:trainData_file_gt.shape[0],
                              :trainData_file_gt.shape[1]] += trainData_file_gt
        ...
        testData_fileList_im2 = sorted(glob(
            os.path.join(
                test_dir,
                dataStructure.testData_subdir_im2,
                cat_tags[0] +
                '_*' +
                dataStructure.im_end)))
