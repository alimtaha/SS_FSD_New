...
class BevParams(object):
    ...
    def convertPositionMetric2Pixel(self, YXpointArrays):
        ...
        allYconverted = self.bev_size[0] - \
            self.meter2px(allY - self.bev_zLimits[0])
        allXconverted = self.meter2px(allX - self.bev_xLimits[0])
        return np.array(
            np.append(
                allYconverted.reshape(
                    (len(allYconverted), 1)), allXconverted.reshape(
                    (len(allXconverted), 1)), axis=1))
        # return [self.meter2px(inputTupleZ + self.bev_zLimits[0]),
        # self.meter2px(inputTupleX + self.bev_xLimits[0])]
    ...
    def convertPositionPixel2Metric(self, YXpointArrays):
        ...
        allY = YXpointArrays[:, 0]
        allX = YXpointArrays[:, 1]
        allYconverted = self.px2meter(
            self.bev_size[0] - allY) + self.bev_zLimits[0]
...
class KittiCalibration(object):
    ...
    def setup(self, dictWithKittiStuff, useRect=False):
        ...
        if useRect:
            ...
            R0_rect_raw = np.array(
                dictWithKittiStuff['R0_rect']).reshape(
                (3, 3))
            # Rectification Matrix
            self.R0_rect = np.matrix(
                np.hstack(
                    (np.vstack(
                        (R0_rect_raw, np.zeros(
                            (1, 3), dtype_str))), np.zeros(
                        (4, 1), dtype_str))))
            self.R0_rect[3, 3] = 1.
            # intermediate result
            R2_1 = np.dot(self.P2, self.R0_rect)

        Tr_cam_to_road_raw = np.array(
            dictWithKittiStuff['Tr_cam_to_road']).reshape(
            3, 4)
        # Transformation matrixs
        self.Tr_cam_to_road = np.matrix(
            np.vstack((Tr_cam_to_road_raw, np.zeros((1, 4), dtype_str))))
...
class BirdsEyeView(object):
    ...
    def __init__(self, bev_res=0.05, bev_xRange_minMax=(-10, 10),
                 bev_zRange_minMax=(6, 46)):
        '''
    ...
    def world2image(self, X_world, Y_world, Z_world):
        ...
        if not isinstance(Y_world, np.ndarray):
            # Y can be a scalar
            Y_world = np.ones_like(Z_world) * Y_world
        y = np.vstack((X_world, Y_world, Z_world, np.ones_like(Z_world)))
        test = self.world2image_uvMat(
            np.vstack((X_world, Z_world, np.ones_like(Z_world))))
        ...
        condition = ~(
            (self.yi1 >= 1) & (
                self.xi1 >= 1) & (
                self.yi1 <= self.imSize[0]) & (
                self.xi1 <= self.imSize[1]))
    ...
    def world2image_uvMat(self, uv_mat):
        ...
        if uv_mat.shape[0] == 2:
            ...
            uv_mat = np.vstack(
                (uv_mat, np.ones((1, uv_mat.shape[1]), uv_mat.dtype)))
    ...
    def computeBEVLookUpTable_reverse(self, imSize=None):
        ...
        yx_im = (mgrid[1:self.imSize_back[0] + 1,
                       1:self.imSize_back[1] + 1]).astype('i4')
        ...
        uvMat = np.vstack(
            (x_im.flatten(), y_im.flatten(), np.ones(
                (dim,), 'f4')))
        xzMat = self.image2world_uvMat(uvMat)
        X = xzMat[0, :].reshape(x_im.shape)
        Z = xzMat[1, :].reshape(x_im.shape)

        XBevInd_reverse_all = np.round(
            (X - self.bevParams.bev_xLimits[0]) / self.bevParams.bev_res).astype('i4')
        ZBevInd_reverse_all = np.round(
            self.bevParams.bev_size[0] -
            (
                Z -
                self.bevParams.bev_zLimits[0]) /
            self.bevParams.bev_res).astype('i4')
        # Valid?
        self.validMapIm_reverse = (
            XBevInd_reverse_all >= 1) & (
            XBevInd_reverse_all <= self.bevParams.bev_size[1]) & (
            ZBevInd_reverse_all >= 1) & (
                ZBevInd_reverse_all <= self.bevParams.bev_size[0])
    ...
    def computeBEVLookUpTable(self, cropping_ul=None, cropping_size=None):
        ...
        x_vec = np.arange(
            self.bevParams.bev_xLimits[0] + res / 2,
            self.bevParams.bev_xLimits[1],
            res)
        z_vec = np.arange(
            self.bevParams.bev_zLimits[1] - res / 2, self.bevParams.bev_zLimits[0], -res)
        ...
        Z_mesh_vec = (
            np.reshape(
                XZ_mesh[1],
                (self.bevParams.bev_size[0] * self.bevParams.bev_size[1]),
                order='F')).astype('f4')
        X_mesh_vec = (
            np.reshape(
                XZ_mesh[0],
        ...
        if (cropping_ul is not None):
            valid_selector = np.ones(
                (self.bevParams.bev_size[0] * self.bevParams.bev_size[1],), dtype='bool')
            valid_selector = valid_selector & (
                self.yi1 >= cropping_ul[0]) & (
                self.xi1 >= cropping_ul[1])
        ...
        ZX_ind = (mgrid[1:self.bevParams.bev_size[0] + 1,
                        1:self.bevParams.bev_size[1] + 1]).astype('i4')
    ...
    def transformImage2BEV(self, inImage, out_dtype='f4'):
        ...
        if len(inImage.shape) > 2:
            outputData = np.zeros(
                self.bevParams.bev_size + (inImage.shape[2],), dtype=out_dtype)
            for channel in xrange(0, inImage.shape[2]):
                outputData[self.bev_z_ind -
                           1, self.bev_x_ind -
                           1, channel] = inImage[self.im_v_float.astype('u4') -
                                                 1, self.im_u_float.astype('u4') -
                                                 1, channel]
        else:
            outputData = np.zeros(self.bevParams.bev_size, dtype=out_dtype)
            outputData[self.bev_z_ind -
                       1, self.bev_x_ind -
                       1] = inImage[self.im_v_float.astype('u4') -
                                    1, self.im_u_float.astype('u4') -
                                    1]
    ...
    def transformBEV2Image(self, bevMask, out_dtype='f4'):
        ...
        if len(bevMask.shape) > 2:
            outputData = np.zeros(self.imSize_back +
                                  (bevMask.shape[2],), dtype=out_dtype)
            for channel in xrange(0, bevMask.shape[2]):
                outputData[self.yImInd_reverse,
                           self.xImInd_reverse,
                           channel] = bevMask[self.ZBevInd_reverse,
                                              self.XBevInd_reverse,
                                              channel]
        else:
            outputData = np.zeros(self.imSize_back, dtype=out_dtype)
            outputData[self.yImInd_reverse,
                       self.xImInd_reverse] = bevMask[self.ZBevInd_reverse,
                                                      self.XBevInd_reverse]
        # Return result
        return outputData
