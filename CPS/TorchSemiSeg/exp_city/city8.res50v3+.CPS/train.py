...
# Commenting out APEX since not using distributed compute
try:
...
with Engine(custom_parser=parser) as engine:
    ...
    train_loader, train_sampler = get_train_loader(
        engine, CityScape, train_source=config.train_source, unsupervised=False)
    unsupervised_train_loader, unsupervised_train_sampler = get_train_loader(
        engine, CityScape, train_source=config.unsup_source, unsupervised=True)
    ...
    if engine.distributed:
        ...
        if torch.cuda.is_available():
            model.cuda()
            # model = DistributedDataParallel(model) #Commented out since not
            # using distributed compute
    else:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        # changed from DataParallelModel to DistributedDataParallel
        model = DistributedDataParallel(model, device_ids=engine.devices)
        model.to(device)
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            cps_loss = criterion_cps(pred_l, max_r) + \
                criterion_cps(pred_r, max_l)
        ...
        if (epoch > config.nepochs // 3) and (epoch %
