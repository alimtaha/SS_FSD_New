...
# All configuration paramters are stored here - including image size,
# training dataset size, etc

# easy dict allows referncing items in a dictionary using the dot notation
# so if "d = edict({'foo':3, 'bar':{'x':1, 'y':2}})", then "d.foo" would
# give you 3
C = edict()
config = C
cfg = C
...
if os.getenv(
        'volna') is not None:  # returns the values of the (bash) environment value 'volna'
    # the authors have used volna as the path to the data directory, to be set in script.sh
    # sets C.volna to the value of the 'volna' environment variable,
    # os.environ returns all the system environment variables as a Py
    # dictionary
    C.volna = os.environ['volna']
else:
    # AUTHOR COMMENT- the path to the data dir.
    C.volna = '/home/cxk/msra_container/'

"""please config ROOT_dir and user when u first using"""  # Needs configuration before running
