...
class NetworkFullResnet(nn.Module):
    def __init__(
        ...
        self.branch1 = SingleNetworkFullResnet(
            num_classes, criterion, norm_layer, pretrained_model, full_depth_resnet=True)
...
class Network(nn.Module):
    def __init__(
        ...
        self.branch1 = SingleNetwork(
            num_classes, criterion, norm_layer, pretrained_model, full_depth_resnet=False)
...
class Head(nn.Module):
    def __init__(
        ...
        self.aspp = ASPP(2048, 256, [12, 18, 24], norm_act=norm_act, full_depth_resnet=full_depth_resnet)
    ...
    def forward(self, f_list, depth):
        ...
        if self.full_depth_resnet == True:
            ...
            embed_feat = torch.concat((f2, d2), dim=1) #embed feats is aspp and depth aspp feats concatenated
        else:
