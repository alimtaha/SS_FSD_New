...
with Engine(custom_parser=parser) as engine:
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        if is_debug:
            ...
            # the command window
            pbar = tqdm(range(1), file=sys.stdout, bar_format=bar_format)
        else:
            pbar = tqdm(
                range(
                    config.niters_per_epoch),
        ...
        for idx in pbar:
            ...
            *- Label doesn't need to be permuted from data loader, however either
            ...
            print('VALIDATING WEAK LABELS VS GT VAL LABELS')
            ...
            with torch.no_grad():
                for batch_test in tqdm(
                    ...
                    print('gowa om el loop')

                    imgs_test = batch_test['data'].to(device)
                    gts_test = batch_test['label'].to(device)
                    #pred_test = model.branch1(imgs_test)
                    weak_image = os.path.join('/home/extraspace/Datasets/Datasets/cityscapes/city/segmentation_weak_0.1/val', batch_test['fn'][0].split('_')[0], ('_').join(batch_test['fn'][0].split('_')[0:3])+'.png')
                    topil = torchvision.transforms.ToTensor()
                    pred_test_max = (np.array(cv2.imread(weak_image, cv2.IMREAD_GRAYSCALE), dtype=np.uint8))
                    
                    for i in range(1, 19):  # setting all labels apart from road to 1 (ignore label)
                        if i == 1: print(np.sum(np.where(pred_test_max == 1)))
                        pred_test_max[np.where(pred_test_max == i)] = 1
                    pred_test_max[np.where(pred_test_max == 255)] = 1

                    pred_test = pred_test_max
                    
                    #loss_sup_test = loss_sup_test + \
                    #    criterion(pred_test_max.unsqueeze(0), gts_test)
