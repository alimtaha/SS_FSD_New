...
import random
import cv2
from exp_city.city8_res50v3_CPS_CutMix.concat.eval_depth_concat import SegEvaluator
from exp_city.city8_res50v3_CPS_CutMix.concat.dataloader_depth_concat import CityScape
from exp_city.city8_res50v3_CPS_CutMix.concat.network_crossattention_depth_concat import Network, count_params
from exp_city.city8_res50v3_CPS_CutMix.concat.dataloader_depth_concat import get_train_loader
...
def main(config, checkpoint_dir=None, engine=None, optim='SGD'):
    ...
    attn_lr_factor = config['attn_lr']
    head_lr_factor = config['head_lr']
    conzeft.weight_decay = config['weight_decay']
    #conzeft.cross_att_mode = config['cross_attn_mode']
    attn_heads = 4 #config['heads']

    #attn_lr_factor = 5
    #head_lr_factor = 5
    notes = 'head-lr-factor' + str(head_lr_factor) + 'attn-lr-factor' + str(attn_lr_factor)
    ...
    if conzeft.weak_labels:
        ...
        '_L' + str(round(conzeft.lr,6)) + str(conzeft.optimiser) + str(conzeft.optim_params) +'_CrossAtt' + str(conzeft.cross_att_mode) + '_ConcatD_' + str(conzeft.image_height) + 'size_' + str(notes)
    else:
    ...
    if conzeft.depthmix:
        import exp_city.city8_res50v3_CPS_CutMix.mask_gen_depth as mask_gen_depth
        ...
        import exp_city.city8_res50v3_CPS_CutMix.mask_gen as mask_gen
    ...
    model = Network(conzeft.num_classes, criterion=criterion,  # change number of classes to free space only
                    ...
                    norm_layer=BatchNorm2d, heads_config=attn_heads)  # need to change norm_layer to nn.BatchNorm2d since BatchNorm2d is derived from the furnace package and doesn't seem to work, it's only needed for syncing batches across multiple GPU, may be needed later
        ...
        for idx in pbar:
            ...
            *- Label doesn't need to be permuted from data loader, however either
            ...
            if step % conzeft.validate_every == 0 or (
                ...
                #tune counts interations every time report is called
                #so make sure to multply num_iters by ratio of iters_per_epoch
                # divided by iters_for_every_val (every time tune.report is called)
    ...
    #logger.add_scalar('train_loss_sup', sum_loss_sup / len(pbar), epoch)
        ...
        # if '''(epoch > conzeft.nepochs // 6) and''' (epoch %
        # conzeft.snapshot_iter == 0) or (epoch == conzeft.nepochs - 1):

        if engine.distributed and (engine.local_rank == 0):
            engine.save_and_link_checkpoint(conzeft.snapshot_dir,
                                        conzeft.log_dir,
                                        conzeft.log_dir_link, epoch)
...
if __name__ == '__main__':
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        result_sgd = tune.run(
            partial(main, engine=engine, optim='SGD'),
