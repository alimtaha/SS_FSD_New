...
class Network(nn.Module):
    ...
    def forward(self, data, contrast, step=1):
        ...
        if step == 1:
            return self.branch1(data)#, contrast)
        elif step == 2:
            return self.branch2(data)#, contrast)
...
class SingleNetwork(nn.Module):
    def __init__(
        ...
        #self.backbone_low = resnet50(pretrained_model, norm_layer=norm_layer,
        #                         bn_eps=config.bn_eps,
        #                         bn_momentum=config.bn_momentum,
        #                         deep_stem=True, stem_width=64, inplace=False)
    ...
    def forward(self, data): #, contrast):
        blocks = self.backbone(data[:, :3, :, :])
        depth = data[:, 3:, :, :]
        #contrast_blocks = self.backbone_low(contrast)
        #v3plus_feature, f_aspp, d_aspp, contrast_aspp = self.head(blocks, depth, contrast_blocks)      # (b, c, h, w)
        v3plus_feature, f_aspp, d_aspp = self.head(blocks, depth)      # (b, c, h, w)
        ...
        if self.training:
            #return v3plus_feature, pred, f_aspp, d_aspp, contrast_blocks
            return v3plus_feature, pred, f_aspp, d_aspp, None
        return pred
...
class ASPP(nn.Module):
    ...
    def forward(self, x, d): #, c):
        ...
        #c1 = F.interpolate(
        #    self.pool_img(c), size=(h, w), mode="bilinear", align_corners=True
        #)
        #c_aspp_list = [m(c) for m in self.map_convs]
        #c_aspp_list.insert(0, c1)
        #c_out = torch.cat(c_aspp_list, dim=1)
        #c_out = self.map_bn(c_out)
        #c_out = self.leak_relu(c_out)       # add activation layer
        #c_out = self.red_conv(c_out)
        #c_out = self.leak_relu(c_out)
...
class Head(nn.Module):
    ...
    def forward(self, f_list, depth): #, contrast_list):
        f = f_list[-1]
        #contrast = contrast_list[-1]
        #f1, depth1, contrast1 = self.aspp(f, depth, contrast)
