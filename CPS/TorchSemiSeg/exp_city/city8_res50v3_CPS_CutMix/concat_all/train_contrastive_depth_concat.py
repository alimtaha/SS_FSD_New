...
from dataloader_contrastive_depth_concat import CityScape
from network_contrastive_depth_concat import Network, count_params
from dataloader_contrastive_depth_concat import get_train_loader
...
with Engine(custom_parser=parser) as engine:
    ...
    criterion = ProbOhemCrossEntropy2d(ignore_label=255, thresh=0.7,  # NUMBER CHANGED TO 5000 from 50000 due to reduction in number of labels since only road labels valid
                                       min_kept=pixel_num, use_weight=False)
    criterion_cps = nn.CrossEntropyLoss(reduction='mean', ignore_index=255)
    criterion_contrast = BYOLContrastiveRegulizer(255, 1, 1, 1)
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            contrast_loss_l = criterion_contrast(img_feat_stu_1, contra_feat_stu_2, depth=None)  * config.contrast_weight
            contrast_loss_r = criterion_contrast(img_feat_stu_2, contra_feat_stu_1, depth=None)  * config.contrast_weight
            

            '''
            NOTE: 
            #ANOTHER OPTION IS COMPUTING THE CONTRAST CRITERION VS THE DIFFERENT BRANCHES 
            # (HI RES ON NETWORK 1 vs LOW-RES AND DEPTH on NETWORK 2) & VICE VERSA

            '''
            

            # supervised loss (either only sup CE or both contrast and CE) on both models
            if config.sup_contrast:
                ...
                contrast_loss_l +=  (criterion_contrast(img_feat_sup_2, contra_feat_sup_1, depth=None)  * config.sup_contrast_weight)
                contrast_loss_r += (criterion_contrast(img_feat_sup_1, contra_feat_sup_2, depth=None)  * config.sup_contrast_weight)
            else:
            ...
            loss = loss_sup + loss_sup_r + cps_loss + contrast_loss_l + contrast_loss_r
            loss.backward()
            ...
            if step % config.validate_every == 0 or (
                ...
                with torch.no_grad():
                    for batch_test in tqdm(
                        ...
                        masked = batch_test['masked'].to(device)
                        pred_test = model.branch1(imgs_test)#, masked)
