...
- Normalisation ( transforming image from 0-255 into 0-1, also performs standardisation of changing mean and std_dev )
...
'''
CHANGE DATALOADER TO MAKE SURE IMAGE PATHS WHEN OPENED ARE REPLACED WITH JPEG INSTEAD OF PNG
'''
...
class TrainPre(
    ...
    def __call__(self, img, dimg=None, gt=None):
        ...
        #   img, dimg, gt, scale = random_scale(img, dimg, gt, config.train_scale_array)
...
class CityScape(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.preprocess is not None:
            img, dimg, gt, edge_gt, extra_dict = self.preprocess(img, dimg, gt)
        if gt is not None:
            for i in range(
        ...
        extra_dict['name'] = names[0].split('/')[3]
