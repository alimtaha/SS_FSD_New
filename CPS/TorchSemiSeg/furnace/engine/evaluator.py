...
from furnace.utils.pyt_utils import load_model, link_file, ensure_dir
from furnace.utils.img_utils import pad_image_to_shape, normalize


'''Changes in lines 277 ans 281'''
...
class Evaluator(object):
    ...
    def scale_process(self, img, ori_shape, crop_size, stride_rate,
        ...
        if long_size <= min(crop_size[0], crop_size[1]):
            input_data, margin = self.process_image(
                img, crop_size)     # pad image
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    input_data, tmargin = self.process_image(
                        img_sub, crop_size)
    ...
    def sliding_eval_rgbdsip(
        ...
        for s in self.multi_scales:
            ...
            new_rows, new_cols, _ = img_scale.shape
            processed_pred += self.scale_process_rgbdisp(
                img_scale,
                disp_scale,
                camera_params,
                (ori_rows,
    ...
    def scale_process_rgbdisp(
        ...
        if long_size <= crop_size:
            input_data, input_disp, margin = self.process_image_rgbd(
                img, disp, crop_size)
            score = self.val_func_process_rgbd(
                input_data, input_disp, camera_params, device)
            score = score[:, margin[0]:(score.shape[1] - margin[1]),
                          margin[2]:(score.shape[2] - margin[3])]
        else:
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    input_data, input_disp, tmargin = self.process_image_rgbd(
                        img_sub, disp_sub, crop_size)
                    temp_score = self.val_func_process_rgbd(
                        input_data, input_disp, camera_params, device)
                    temp_score = temp_score[:,
                                            tmargin[0]:(temp_score.shape[1] - tmargin[1]),
                                            tmargin[2]:(temp_score.shape[2] - tmargin[3])]
        ...
        with torch.cuda.device(input_data.get_device()):
            ...
            with torch.no_grad():
                ...
                if self.is_flip:
                    ...
                    score_flip = self.val_func(
                        input_data, input_disp, camera_params)
                    score_flip = score_flip[0]
                    score += score_flip.flip(-1)
                score = torch.exp(score)
                # score = score.data
    ...
    def sliding_eval_rgbdepth(
        ...
        for s in self.multi_scales:
            ...
            new_rows, new_cols, _ = img_scale.shape
            processed_pred += self.scale_process_rgbdepth(
                img_scale, disp_scale, (ori_rows, ori_cols), crop_size, stride_rate, device)

        pred = processed_pred.argmax(2)

        return pred

    def scale_process_rgbdepth(
            self,
            img,
            disp,
            ori_shape,
        ...
        if long_size <= crop_size:
            input_data, input_disp, margin = self.process_image_rgbd(
                img, disp, crop_size)
            score = self.val_func_process_rgbdepth(
                input_data, input_disp, device)
            score = score[:, margin[0]:(score.shape[1] - margin[1]),
                          margin[2]:(score.shape[2] - margin[3])]
        else:
            stride = int(np.ceil(crop_size * stride_rate))
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    input_data, input_disp, tmargin = self.process_image_rgbd(
                        img_sub, disp_sub, crop_size)
                    temp_score = self.val_func_process_rgbdepth(
                        input_data, input_disp, device)
