...
def load_model(model, model_file, is_restore=False):
    ...
    if isinstance(
            model_file,
            str):  # if model_file passed in is either a file/file-like object or a file name/directory
        state_dict = torch.load(model_file)
        if 'model' in state_dict.keys(
        ):  # if the state dict has more than just the model (so a dictionary with multiple keys) we only want the 'model' key
            state_dict = state_dict['model']
    else:
        # if the passed in model file was actually the state dict directly and
        # not a filename/path
