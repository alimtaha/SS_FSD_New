...
def log_det_by_cholesky(matrix):
    ...
    # return 2.0 * torch.sum(torch.log(torch.diagonal(chol, dim1=-2, dim2=-1)
    # + 1e-6), dim=-1)
    return 2.0 * \
        torch.sum(torch.log(torch.diagonal(chol, dim1=-2, dim2=-1) + 1e-8), dim=-1)
...
class RMILoss(nn.Module):
    ...
    def forward_sigmoid(self, logits_4D, labels_4D):
        ...
        valid_onehot_labels_4D = convert_to_onehot(
            labels_4D.long(), num_classes=self.num_classes).float()
        ...
        final_loss = (self.weight_lambda * bce_loss + rmi_loss * (1 - self.weight_lambda)
                      if self.lambda_way else bce_loss + rmi_loss * self.weight_lambda)
    ...
    def rmi_lower_bound(self, labels_4D, probs_4D):
        ...
        # small diagonal matrix, shape = [1, 1, radius * radius, radius *
        # radius]
        ...
        # If A = A^T, A^-1 = (A^-1)^T.
        appro_var = la_cov - \
            torch.matmul(la_pr_cov.matmul(pr_cov_inv), la_pr_cov.transpose(-2, -1))
        ...
        rmi_now = 0.5 * \
            log_det_by_cholesky(appro_var + diag_matrix.type_as(appro_var) * _POS_ALPHA)
