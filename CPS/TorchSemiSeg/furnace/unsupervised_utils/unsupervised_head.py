...
def guided_cutout(
    ...
    for mask in masks:
        ...
        except BaseException:  # Version 4.x
...
def softmax_mse_loss(
        ...
        threshold=None,
        use_softmax=False):
    assert inputs.requires_grad and targets.requires_grad == False
...
class abCE_loss(nn.Module):
    ...
    def forward(self, predict, target, ignore_index, curr_iter, epoch):
        ...
        sort_prob, sort_indices = prob.contiguous(
        ).view(-1, )[mask].contiguous().sort()
        ...
        min_threshold = sort_prob[min(
            batch_kept, sort_prob.numel() - 1)] if sort_prob.numel() > 0 else 0.0
        threshold = max(min_threshold, thresh)
        loss_matrix = F.cross_entropy(
            predict,
            target,
            weight=self.weight.to(
                predict.device) if self.weight is not None else None,
