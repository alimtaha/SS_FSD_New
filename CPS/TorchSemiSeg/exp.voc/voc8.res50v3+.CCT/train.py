...
with Engine(custom_parser=parser) as engine:
    ...
    criterion = abCE_loss(
        iters_per_epoch=config.niters_per_epoch,
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            loss_sup, loss_unsup = model(
                imgs, unsupervised_imgs, gts, curr_iter=idx + 1, epoch=epoch)
        ...
        if engine.distributed and (engine.local_rank == 0):
            ...
            if azure:
                ...
                run.log(
                    name='Unsupervised Training Loss',
                    value=sum_loss_unsup / len(pbar))
