...
class Network(nn.Module):
    def __init__(
        ...
        drop_decoder = [
            DropOutDecoder(
                ...
                norm_layer=norm_layer) for _ in range(
                conf['drop'])]
        cut_decoder = [
            CutOutDecoder(
                upscale,
    ...
    def forward(
        ...
        outputs_unsup = [
            aux_decoder(
                encoder_unsup,
                output_unsup_main.detach()) for aux_decoder in self.aux_decoders]
        targets = F.softmax(output_unsup_main.detach(), dim=1)

        # Compute unsupervised loss
        loss_unsup = sum([self.unsuper_loss(inputs=u,
                                            targets=targets,
