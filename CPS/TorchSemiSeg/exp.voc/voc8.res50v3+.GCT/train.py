...
with Engine(custom_parser=parser) as engine:
    ...
    train_loader, train_sampler = get_train_loader(
        engine, VOC, train_source=config.train_source, unsupervised=False)
    unsupervised_train_loader, unsupervised_train_sampler = get_train_loader(
        engine, VOC, train_source=config.unsup_source, unsupervised=True)
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            # set the gt of unlabeled data to be 255
            gts_unlabeled = (gts * 0 + 255).long()
            ...
            pred_l, flawmap_l, l_fc_mask, l_dc_gt, pred_r, flawmap_r, r_fc_mask, r_dc_gt = model(
                l_inp, r_inp, l_gt, r_gt, current_idx, total_steps, step=1)
            ...
            fc_ssl_loss_l = fd_criterion(
                flawmap_l,
                torch.zeros(
                    flawmap_l.shape).cuda(),
                is_ssl=True,
                reduction=False)
            ...
            dc_ssl_loss_l = dc_rampup_scale * \
                config.dc_ssl_scale * torch.mean(dc_ssl_loss_l)
            ...
            fc_ssl_loss_r = fd_criterion(
                flawmap_r,
                torch.zeros(
                    flawmap_r.shape).cuda(),
                is_ssl=True,
                reduction=False)
            ...
            dc_ssl_loss_r = dc_rampup_scale * \
                config.dc_ssl_scale * torch.mean(dc_ssl_loss_r)
            ...
            l_flawmap, r_flawmap, l_flawmap_gt, r_flawmap_gt = model(
                l_inp, r_inp, l_gt, r_gt, current_idx, total_steps, step=2)

            # generate the ground truth for the flaw detector (on labeled data
            # only)
            lbs = b // 2
        ...
        if (epoch > config.nepochs // 2) and (epoch %
