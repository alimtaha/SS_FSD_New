...
class Network(nn.Module):
    ...
    def forward(
        ...
        if step == 1:
            ...
            lbs = b // 2
            with torch.no_grad():
                l_flawmap_gt = self.fdgt_generator(l_activated_pred[:lbs, ...].detach(
                ), sslgct_prepare_task_gt_for_fdgt(l_gt[:lbs, ...]))
                r_flawmap_gt = self.fdgt_generator(r_activated_pred[:lbs, ...].detach(
                ), sslgct_prepare_task_gt_for_fdgt(r_gt[:lbs, ...]))
...
class ASPP(nn.Module):
    def __init__(self,
        ...
        self.red_conv = nn.Conv2d(
            hidden_channels * 4, out_channels, 1, bias=False)
...
class Head(nn.Module):
    def __init__(
        ...
        self.last_conv = nn.Sequential(
            nn.Conv2d(
                304, 256, kernel_size=3, stride=1, padding=1, bias=False), norm_act(
