...
with Engine(custom_parser=parser) as engine:
    ...
    train_loader, train_sampler = get_train_loader(
        engine, VOC, train_source=config.train_source, unsupervised=False, collate_fn=collate_fn)
    unsupervised_train_loader_0, unsupervised_train_sampler_0 = get_train_loader(
        engine, VOC, train_source=config.unsup_source, unsupervised=True, collate_fn=mask_collate_fn)
    unsupervised_train_loader_1, unsupervised_train_sampler_1 = get_train_loader(
        engine, VOC, train_source=config.unsup_source, unsupervised=True, collate_fn=collate_fn)
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            cps_loss = criterion(logits_cons_stu_1, ps_label_2) + \
                criterion(logits_cons_stu_2, ps_label_1)
        ...
        if azure and engine.local_rank == 0:
            ...
            run.log(
                name='Supervised Training Loss right',
                value=sum_loss_sup_r / len(pbar))
            run.log(
                name='Supervised Training Loss CPS',
                value=sum_cps / len(pbar))
