...
class VOC(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # train or train_aug or xxx
        datadir = workdir.split('/')[-1].split('.')[0]
        if 'train' in datadir:
            datadir = 'train_aug'
        ...
        if self.pseudo_label is True:
            img_path = os.path.join(
                self._img_path, datadir, 'image', names + '.jpg')
            print(img_path)
        ...
        if 'test' in datadir:
            ...
            img_path = os.path.join(
                self._img_path, datadir, 'image', sp[0] + '.jpg')

        if './' not in names:
            gt_path = os.path.join(
                self._gt_path, datadir, 'label', sp[0] + '.png')
