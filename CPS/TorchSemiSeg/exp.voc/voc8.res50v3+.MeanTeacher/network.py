    ...
    def forward(self, data, step=1, cur_iter=None):
        ...
        if cur_iter == 0:
            for t_param, s_param in zip(
                    self.branch2.parameters(), self.branch1.parameters()):
                t_param.data.copy_(s_param.data)

        s_feature, s_out = self.branch1(data)

        with torch.no_grad():
            t_feature, t_out = self.branch2(data)

        if step == 1:
            self._update_ema_variables(config.ema_decay, cur_iter)
