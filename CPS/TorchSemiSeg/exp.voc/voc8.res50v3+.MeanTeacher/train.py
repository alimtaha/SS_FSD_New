...
with Engine(custom_parser=parser) as engine:
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            s_unsup_pred, t_unsup_pred = model(
                unsup_imgs, step=2, cur_iter=current_idx)
            ...
            loss.backward()
            # only the student model need to be updated by SGD.
            optimizer_l.step()
        ...
        if azure and engine.local_rank == 0:
            run.log(
                name='Supervised Training Loss',
                value=sum_loss_sup / len(pbar))
            run.log(
                name='Supervised Training Loss Teacher',
                value=sum_loss_sup_t / len(pbar))
            run.log(
                name='Supervised Training Loss CSST',
                value=sum_csst / len(pbar))
