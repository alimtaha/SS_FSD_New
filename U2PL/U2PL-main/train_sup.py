...
def viz_image(img, gts, pred, step, mode, tb_logger=None, name=None):
    image_viz = img[0, :, :, :].squeeze().cpu().numpy() * np.expand_dims(np.expand_dims(
        std, axis=1), axis=2) + np.expand_dims(np.expand_dims(mean, axis=1), axis=2)
...
def main():
    global args, cfg, prototype, mean, std, colormap, i_iter
    ...
    experiment_name = str(cfg["trainer"]["epochs"]) + 'E_FS' + str(cfg["dataset"][
        "n_sup"]) + '_L' + str(cfg["trainer"]["optimizer"]["kwargs"]["lr"]) + '_NoD'
    ...
    for epoch in range(last_epoch, cfg_trainer["epochs"]):
        ...
        prec = validate(model, val_loader, epoch, i_iter, tb_logger, logger)
...
def train(
    ...
    for step in range(len(data_loader)):
        ...
        image, label, name = data_loader_iter.next()
        ...
        if i_iter % 10 == 0 and rank == 0:
            ...
            if i_iter % 100 == 0:
                viz_image(image, label.squeeze().cpu(), pred.data.max(
...
def validate(
    ...
    for step, batch in enumerate(data_loader):
        ...
        output = F.interpolate(
            output, (h, w), mode="bilinear", align_corners=True)
