...
model_urls = {
    ...
    "resnet50": "/home/taha_a@WMGDS.WMG.WARWICK.AC.UK/Documents/ss_fsd/CPS/TorchSemiSeg/DATA/pytorch-weight/resnet50_v1c.pth",
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.bn1 = norm_layer(width, eps=bn_eps, momentum=bn_momentum)
        self.conv2 = conv3x3(width, width, stride, groups, dilation)
        self.bn2 = norm_layer(width, eps=bn_eps, momentum=bn_momentum)
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Sequential(
            ...
            norm_layer(64, eps=bn_eps, momentum=bn_momentum),
        ...
        self.bn1 = norm_layer(self.inplanes, eps=bn_eps, momentum=bn_momentum)
        self.relu = nn.ReLU(inplace=inplace)
    ...
    def _make_layer(
        ...
        multi_grid=False, inplace=True, bn_eps=1e-5, bn_momentum=0.1
    ):
...
def resnet50(pretrained=True, **kwargs):
    ...
    for m in model.layer4.children():
        m.apply(partial(_nostride_dilate, dilate=dilate))  # the apply function only takes in references to functions and then calls the function after, it expects whatever function is passed to it to have only one parameter, m, as the model. It then calls the function with m as the parameter. The reason partial is used is because the _nostride_dilate function has more than one input argument, so to get over that, the partial class takes in a function, and a partial subset of it's input arguments, and wraps a new function around them where the new function's input parameters are what remains (what wasn't defined in the partial call). So the apply function only sees a function with one input argument, m, so it works.
        dilate *= 2

    return model


def resnet50_depth_append(pretrained=True, **kwargs):
    """Constructs a ResNet-50 model which takes 4 dimensions as an input (depth maps).
