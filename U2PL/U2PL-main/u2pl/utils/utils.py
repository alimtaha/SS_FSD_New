...
def uns_crops(length, crop, img_size, ignore_label):
    uns_crops = []
    h = img_size[0]
    w = img_size[1]
    crop_h = crop[0]
    ...
    for i in range(length):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            label = F.pad(label, border, mode="constant", value=ignore_label)
        h, w = image.size()[-2:]
        h_off = random.randint(0, h - crop_h)
        w_off = random.randint(0, w - crop_w)
        uns_crops.append()(h_off, w_off)
...
def generate_cutmix(pred, cat, area_thresh, no_pad=False, no_slim=False):
    ...
    if flag == 1:
        ...
        area = (rectangles[2] - rectangles[0]) * \
...
class AverageMeter(object):
    ...
    def update(self, val, num=1):
        if self.length > 0:
            # currently assert num==1 to avoid bad usage, refine when there are
            # some explict requirements
            assert num == 1
...
def recall_accuracy_precision(output, target, n_cl):
    assert output.ndim in [1, 2, 3]
    assert output.shape == target.shape
    output = output.reshape(output.size).copy()
    target = target.reshape(target.size)
    k = (target >= 0) & (target < n_cl)
    accuracy = [0] * n_cl
    precision = [0] * n_cl
    recall = [0] * n_cl
    mean_prec = 0
    ...
    mean_acc = 0
    for i in range(n_cl):
        tp = np.sum((output[k] == i) & (target[k] == i))
        fp = np.sum((output[k] == i) & (target[k] != i))
        tn = np.sum((output[k] != i) & (target[k] != i))
        fn = np.sum((output[k] != i) & (target[k] == i))
    ...
    return precision, mean_prec, recall, mean_recall, accuracy, mean_acc
