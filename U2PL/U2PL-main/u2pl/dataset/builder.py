...
from .cityscapes import build_city_semi_loader, build_cityloader, build_city_semi_depth_loader, build_cityloader_depth
...
def get_loader(cfg, seed=0):
    ...
    if cfg_dataset["type"] == "cityscapes_semi":
        train_loader_sup, train_loader_unsup, train_loader_aug_unsup = build_city_semi_loader(
            "train", cfg, seed=seed)
        val_loader = build_cityloader("val", cfg)
        logger.info("Get loader Done...")
        return train_loader_sup, train_loader_unsup, train_loader_aug_unsup, val_loader

    if cfg_dataset["type"] == "cityscapes_semi_depth":
        train_loader_sup, train_loader_unsup, train_loader_aug_unsup = build_city_semi_depth_loader(
            "train", cfg, seed=seed)
        val_loader = build_cityloader_depth("val", cfg)
        logger.info("Get loader Done...")
        return train_loader_sup, train_loader_unsup, train_loader_aug_unsup, val_loader
