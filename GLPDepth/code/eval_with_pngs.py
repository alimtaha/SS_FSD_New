...
parser = argparse.ArgumentParser(
    description='Evaluate Depth Estimation Results',
...
parser.add_argument(
    '--pred_path',
    type=str,
    help='path to the prediction results in png',
    required=True)
parser.add_argument(
    '--gt_path',
    type=str,
    help='root path to the groundtruth data',
    required=True)
parser.add_argument(
    '--dataset',
    type=str,
    help='dataset to test on, nyudepthv2 or kitti',
    default='nyudepthv2',
    choices=[
        'kitti',
         'nyudepthv2'])
...
def test():
    ...
    gt_depths = []
    missing_ids = set()
    pred_filenames = [i for i in os.listdir(
        args.pred_path) if i.split('.')[-1] in ['jpg', 'png']]
...
def eval(pred_depths):
    ...
    for i in trange(num_samples):
        ...
        if args.do_kb_crop:
            ...
            gt_cropped = gt_depth[top_margin:top_margin +
