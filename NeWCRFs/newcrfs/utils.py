...
def compute_errors(gt, pred):
    ...
    for i, mask in enumerate(masks):
        ...
        rms = (gt[mask] - pred[mask]) ** 2
        rms = np.sqrt(rms.mean())

        log_rms = (np.log(gt[mask]) - np.log(pred[mask])) ** 2
        ...
        err = np.abs(np.log10(pred[mask]) - np.log10(gt[mask]))
        log10 = np.mean(err)

        all_results[mask_names[i]] = dict(silog=silog,
                                           abs_rel=abs_rel,
                                           log10=log10,
                                           rms=rms,
                                           sq_rel=sq_rel,
...
class silog_loss(nn.Module):
    ...
    def forward(self, depth_est, depth_gt, mask):
        ...
        return torch.sqrt((d ** 2).mean() -
                          self.variance_focus * (d.mean() ** 2)) * 10.0
