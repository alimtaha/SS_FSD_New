...
parser.add_argument(
    '--dataset',
    type=str,
    help='dataset this model trained on',
    default='nyu')
parser.add_argument(
    '--crop',
    type=str,
    help='crop: kbcrop, edge, non',
    default='non')
...
camera_matrix[2, 2] = 1
dist_coeffs = np.array([3.7230261423972011e-02, -1.6171708069773008e-01, -
                        3.5260752900266357e-04, 1.7161234226767313e-04, 1.0192711400840315e-01])
...
map1, map2 = cv2.initUndistortRectifyMap(
    camera_matrix, dist_coeffs, R, new_camera_matrix, (width_rgb, height_rgb), cv2.CV_32FC1)
...
class Window(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        if self.glWidget.rgb.any() and self.glWidget.depth.any():
            ...
            self.inputViewer.setPixmap(
                QtGui.QPixmap.fromImage(
                    np_to_qimage(img)))
            coloredDepth = (plasma(self.glWidget.depth[:, :, 0])[
                            :, :, :3] * 255).astype('uint8')
            self.outputViewer.setPixmap(
                QtGui.QPixmap.fromImage(
                    np_to_qimage(coloredDepth)))
    ...
    def loadImageFile(self):
        ...
        filename = QtWidgets.QFileDialog.getOpenFileName(
            None, 'Select image', '', self.tr('Image files (*.jpg *.png)'))[0]
        ...
        if self.model:
            ...
            with torch.no_grad():
                ...
                if post_process:
                    ...
                    depth_cropped = post_process_depth(
                        depth_est, depth_est_flipped)

            depth = np.zeros((height_depth, width_depth), dtype=np.float32)
            if args.crop == 'kbcrop':
                depth[top_margin:top_margin + 352, left_margin:left_margin +
                      1216] = depth_cropped[0].cpu().squeeze() / args.max_depth
            elif args.crop == 'edge':
                depth[32:-32, 32:-32] = depth_cropped[0].cpu().squeeze() / \
                    args.max_depth
            else:
            ...
            self.glWidget.depth = 0.5 + \
                np.zeros((height_rgb // 2, width_rgb // 2, 1))
...
class GLWidget(QtOpenGL.QGLWidget):
    ...
    def drawObject(self):
        ...
        model, view, proj = glm.mat4(1), glm.mat4(1), glm.perspective(
            45, self.width() / self.height(), 0.01, 100)
        center, up, eye = glm.vec3(
            0, -0.075, 0), glm.vec3(0, -1, 0), glm.vec3(0, 0, -0.4 * (self.zoomLevel / 10))
