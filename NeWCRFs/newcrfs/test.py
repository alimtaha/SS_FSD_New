...
print('RUNNING ON CPU - TURN ON GPU ACCELERATION FOR TEST SCRIPT BY UNCOMMENTING .CUDA AND REMOVE MAP LOCATION FROM TORCH.LOAD')
print('remove break in model loop and saving loop')
...
parser.add_argument(
    '--save_viz',
    help='if set, save visulization of the outputs',
    action='store_true')
...
def test(params):
    ...
    print('now testing {} files with {}'.format(
        num_test_samples, args.checkpoint_path))
    ...
    with torch.no_grad():
        for i, sample in enumerate(tqdm(dataloader.data)):
            if i > 3:
                break
            image = Variable(sample['image'].cpu())  # .cuda())
            # Predict
    ...
    for s in tqdm(range(num_test_samples)):
        ...
        if args.dataset == 'kitti':
            ...
            filename_pred_png = save_name + '/raw/' + date_drive + '_' + \
                lines[s].split()[0].split('/')[-1].replace('.jpg', '.png')
            filename_cmap_png = save_name + '/cmap/' + date_drive + '_' + \
                lines[s].split()[0].split('/')[-1].replace('.jpg', '.png')
            filename_image_png = save_name + '/rgb/' + \
                date_drive + '_' + lines[s].split()[0].split('/')[-1]
        elif args.dataset == 'kittipred':
            filename_pred_png = save_name + '/raw/' + \
                lines[s].split()[0].split('/')[-1].replace('.jpg', '.png')
            filename_cmap_png = save_name + '/cmap/' + \
                lines[s].split()[0].split('/')[-1].replace('.jpg', '.png')
            filename_image_png = save_name + '/rgb/' + \
            ...
            print(lines[s])
            filename_pred_png = save_name + '/raw/' + scene_name + '_' + \
                lines[s].split()[0].split('/')[-1].replace('.jpg', '.png')
            filename_cmap_png = save_name + '/cmap/' + scene_name + '_' + \
                lines[s].split()[0].split('/')[-1].replace('.jpg', '.png')
            filename_gt_png = save_name + '/gt/' + scene_name + '_' + \
                lines[s].split()[0].split('/')[-1].replace('.jpg', '_gt.png')
            filename_image_png = save_name + '/rgb/' + \
                scene_name + '_' + lines[s].split()[0].split('/')[-1]
        ...
        if args.dataset == 'kitti' or args.dataset == 'kittipred' or args.dataset == 'cityscapes':
        ...
        cv2.imwrite(
            filename_pred_png, pred_depth_scaled, [
