...
def main_worker(gpu, ngpus_per_node, args):
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in tqdm(
            ...
            if args.dataset == 'nyu':
                mask = depth_gt > args.min_depth_train      #changed from 0.01
            else:
                mask = depth_gt > args.min_depth_train      #changed from 0.001
            ...
            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and not model_just_loaded and rank == 0:
                time.sleep(0.1)
                model.eval()
                mask_names = [
                    'mask_20',
...
def main():
    ...
    if save_files:
        ...
        os.system(command)
        command = 'mkdir -p ' + networks_savepath + \
            ' && cp newcrfs/networks/*.py ' + networks_savepath
        os.system(command)
        command = 'mkdir -p ' + dataloaders_savepath + \
            ' && cp newcrfs/dataloaders/*.py ' + dataloaders_savepath
        os.system(command)
