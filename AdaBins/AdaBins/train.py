...
def main_worker(gpu, ngpus_per_node, args):
    ...
    experiment_name = 'AdaBins' + str(args.epochs) + 'Epochs_' + str(args.lr) + 'LR_' + str(args.input_height) + 'x' + str(
...
def train(
    ...
    for epoch in range(args.epoch, epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Train",
            ...
            clip  = get_grad_clip(step, 2, 0.1, decay_rate = 0.9)
            ...
            if should_write and step % args.validate_every == 0:
                ...
                if metrics['mask_all']['abs_rel'] < best_loss and should_write:
                    model_io.save_checkpoint(
                        ...
                        root=tb_dir)
                    best_loss = metrics['mask_all']['abs_rel']

                for k, v in h_params_metrics.items():
                    if k in ['d1', 'd2', 'd3']:
                        if v > metrics['mask_all'][k]:
                            continue
                        else:
                            h_params_metrics[k] = metrics['mask_all'][k]
                    else:
                        if v < metrics['mask_all'][k]:
                            continue
                        else:
                ...
                writer.add_hparams(hparams_dict, h_params_metrics, run_name=experiment_name)
...
def validate(
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(
                test_loader,
                desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Validation") if is_rank_zero(args) else test_loader:
            img = batch['image'].to(device)
            depth = batch['depth'].to(device)
